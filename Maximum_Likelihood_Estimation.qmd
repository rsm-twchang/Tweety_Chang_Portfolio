...
| `iscustomer` | Group        | Mean patents (5 years) |
|------------:|--------------|------------------------:|
...
```{python}
#| echo: false
#| fig-cap: "Figure Y. Incidence rate ratios (IRR = exp(coef)) with 95% confidence intervals from the Poisson GLM."
#| label: fig-irr
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
...
# Choose terms to display (you can add/remove)
terms = ["iscustomer", "age", "age_squared",
         "region_Northeast", "region_Northwest", "region_South", "region_Southwest"]

irr = np.exp(coefs[terms])
irr_lo = np.exp(ci.loc[terms, "lo"])
irr_hi = np.exp(ci.loc[terms, "hi"])
...
y = np.arange(len(df_irr))
xerr = np.vstack([df_irr["IRR"] - df_irr["lo"], df_irr["hi"] - df_irr["IRR"]])

plt.figure(figsize=(7.5, 4.8))
plt.errorbar(df_irr["IRR"], y, xerr=xerr, fmt="o")
plt.axvline(1, linestyle="--")
plt.yticks(y, df_irr["term"])
plt.xlabel("Incidence Rate Ratio (IRR)")
plt.title("IRR with 95% CI")
plt.grid(True)
plt.tight_layout()
plt.show()
```
